/*
 ...
 Description:  This script gives the list of schedules created/available in 
               SQL Server and the details (Occurrence, Recurrence, Frequency, etc.) 
...
SELECT schedule_uid AS ScheduleID,
       ...
       CASE freq_subday_type
            ...
            WHEN 2 THEN 'Occurs every ' + CAST(freq_subday_interval AS VARCHAR(3)) + ' Second(s) between ' 
			          + STUFF(STUFF(RIGHT('000000' + CAST(active_start_time AS VARCHAR(6)), 6), 3, 0, ':'), 6, 0, ':')
                      + ' & ' 
                      + STUFF(
                        STUFF(RIGHT('000000' + CAST(active_end_time AS VARCHAR(6)), 6), 3, 0, ':'), 6, 0, ':')
            WHEN 4 THEN 'Occurs every ' + CAST(freq_subday_interval AS VARCHAR(3)) + ' Minute(s) between ' 
                      + STUFF(STUFF(RIGHT('000000' + CAST(active_start_time AS VARCHAR(6)), 6), 3, 0, ':'), 6, 0, ':')
                      + ' & ' 
                      + STUFF(
                        STUFF(RIGHT('000000' + CAST(active_end_time AS VARCHAR(6)), 6), 3, 0, ':'), 6, 0, ':')
            WHEN 8 THEN 'Occurs every ' + CAST(freq_subday_interval AS VARCHAR(3)) + ' Hour(s) between ' 
                      + STUFF(STUFF(RIGHT('000000' + CAST(active_start_time AS VARCHAR(6)), 6), 3, 0, ':'), 6, 0, ':')
                      + ' & ' 
                      + STUFF(
                        STUFF(RIGHT('000000' + CAST(active_end_time AS VARCHAR(6)), 6) , 3, 0, ':'), 6, 0, ':')
 ...
 WHERE 1 = 1
   --AND (name NOT LIKE 'Daily%' AND name NOT LIKE 'Every%' AND name NOT LIKE 'Once%' AND name NOT LIKE 'One%' AND name NOT LIKE 'Weekly%')
