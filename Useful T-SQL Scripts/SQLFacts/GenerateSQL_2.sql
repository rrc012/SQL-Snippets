...
DECLARE @SchemaName1 varchar(0128) = 'dbo'
DECLARE @ObjectName1 varchar(0128) = 'Name1' -- table/view/TVF for query

DECLARE @SchemaName2 varchar(0128) = 'dbo'
DECLARE @ObjectName2 varchar(0128) = 'Name2' -- table/view/TVF for query
...
DECLARE @PathID              int
DECLARE @PathSize       smallint
DECLARE @PathVast  varchar(7000)
DECLARE @PathLast  varchar(0256)

CREATE TABLE #Path
     ( PathID              int NOT NULL IDENTITY(1,1)
     , PathSize       smallint NOT NULL
     , PathVast  varchar(7000) NOT NULL
     , PathLast  varchar(0256) NOT NULL )
...
-- generate SQL
   ...
   INSERT #Path
        ...
        , PathLast )
   SELECT @T
        , '|' + F.PrimarySchema + '.' + F.PrimaryObject + '|' + F.ForeignSchema + '.' + F.ForeignObject + '|'
        ,                                                       F.ForeignSchema + '.' + F.ForeignObject
     FROM #FKeys AS F
    WHERE F.PrimarySchema = @SchemaName1
      AND F.PrimaryObject = @ObjectName1
 ORDER BY 2
   ...
   INSERT #Path
        ...
        , PathLast )
   SELECT @T
        , '|' + F.ForeignSchema + '.' + F.ForeignObject + '|' + F.PrimarySchema + '.' + F.PrimaryObject + '|'
        ,                                                       F.PrimarySchema + '.' + F.PrimaryObject
     FROM #FKeys AS F
    WHERE F.ForeignSchema = @SchemaName1
      AND F.ForeignObject = @ObjectName1
 ORDER BY 2

   SELECT @U = COUNT(*)
     FROM #Path AS P
    WHERE P.PathLast = @SchemaName2 + '.' + @ObjectName2
...
WHILE @U = 0 AND @T < 50
    ...
    BEGIN
       ...
       INSERT #Path
            ...
            , PathLast )
       SELECT P.PathSize + 1
            , P.PathVast + F.ForeignSchema + '.' + F.ForeignObject + '|'
            ,              F.ForeignSchema + '.' + F.ForeignObject
         FROM #FKeys AS F
         JOIN #Path  AS P
           ON P.PathSize = @T
          AND P.PathLast = F.PrimarySchema + '.' + F.PrimaryObject
     ORDER BY 2
       ...
       INSERT #Path
            ...
            , PathLast )
       SELECT P.PathSize + 1
            , P.PathVast + F.PrimarySchema + '.' + F.PrimaryObject + '|'
            ,              F.PrimarySchema + '.' + F.PrimaryObject
         FROM #FKeys AS F
         JOIN #Path  AS P
           ON P.PathSize = @T
          AND P.PathLast = F.ForeignSchema + '.' + F.ForeignObject
     ORDER BY 2
   ...
   SELECT @V = MIN(P.PathSize)
     FROM #Path AS P
    WHERE P.PathLast = @SchemaName2 + '.' + @ObjectName2

   SELECT REPLACE(SUBSTRING(P.PathVast, 2, LEN(P.PathVast) - 2), '|', ', ') AS QueryPath
     FROM #Path AS P
    WHERE P.PathLast = @SchemaName2 + '.' + @ObjectName2
      AND P.PathSize = @V
 ORDER BY P.PathID
...
FETCH NEXT FROM Objects INTO @PathID, @PathSize, @PathVast, @PathLast

WHILE @@FETCH_STATUS = 0

    BEGIN

    PRINT '-- The query below uses: ' + REPLACE(SUBSTRING(@PathVast, 2, LEN(@PathVast) - 2), '|', ', ')
    ...
    SET @SQLServerCode = '   SELECT '
      ...
      DECLARE DBItems CURSOR FAST_FORWARD FOR
       SELECT E.GeneralID
            ...
            , E.I
         FROM #Base AS E
        WHERE CHARINDEX('|' + E.GeneralSchema + '.' + E.GeneralObject + '|', @PathVast) > 0
     ORDER BY CHARINDEX('|' + E.GeneralSchema + '.' + E.GeneralObject + '|', @PathVast)

    OPEN DBItems

    FETCH NEXT FROM DBItems INTO @VirtualID, @GeneralSchema, @GeneralObject, @SQLServerMore, @Z
    ...
    WHILE @@FETCH_STATUS = 0
        ...
        BEGIN
          ...
          DECLARE Columns CURSOR FAST_FORWARD FOR
           SELECT E.SQLServerName
                , T.GeneralColumn
             FROM #Base AS E
             JOIN #TKey AS T
               ON E.GeneralID
                = T.GeneralID
            WHERE E.GeneralSchema = @GeneralSchema
              AND E.GeneralObject = @GeneralObject
         ORDER BY T.column_id

        OPEN Columns

        FETCH NEXT FROM Columns INTO @SQLServerName, @GeneralColumn
      ...
      DECLARE Columns CURSOR FAST_FORWARD FOR
       SELECT E.SQLServerName
         ...
         JOIN #TKey AS T
           ON E.GeneralID
            = T.GeneralID
        WHERE E.GeneralSchema = @SchemaName1
          AND E.GeneralObject = @ObjectName1
     ORDER BY T.column_id
      ...
      DECLARE Columns CURSOR FAST_FORWARD FOR
       SELECT E.SQLServerName
         ...
         JOIN #TKey AS T
           ON E.GeneralID
            = T.GeneralID
        WHERE E.GeneralSchema = @SchemaName2
          AND E.GeneralObject = @ObjectName2
     ORDER BY T.column_id
...
*/
    ...
    PRINT '     FROM ' + @SchemaName1 + '.' + @X + @ObjectName1 + @Y + @Parameters + SPACE(@I - @W) + ' AS ' + @SQLServerName
      ...
      DECLARE DBItems CURSOR FAST_FORWARD FOR
       SELECT E.GeneralID
            ...
            , E.I
         FROM #Base AS E
        WHERE CHARINDEX('|' + E.GeneralSchema + '.' + E.GeneralObject + '|', @PathVast) > 1
     ORDER BY CHARINDEX('|' + E.GeneralSchema + '.' + E.GeneralObject + '|', @PathVast)

    OPEN DBItems

    FETCH NEXT FROM DBItems INTO @VirtualID, @GeneralSchema, @GeneralObject, @SQLServerMore, @Parameterz, @Z

    WHILE @@FETCH_STATUS = 0

        BEGIN

        PRINT '     JOIN ' + @GeneralSchema + '.' + @X + @GeneralObject + @Y + @Parameterz + SPACE(@I - @Z) + ' AS ' + @SQLServerMore
    ...
    SET @SQLServerCode = '       ON '
        ...
        WHILE @@FETCH_STATUS = 0
            ...
            PRINT @SQLServerCode + @SQLServerName + '.' + @X + @PrimaryColumn + @Y + CHAR(13) + CHAR(10) + '        = ' + @SQLServerMore + '.' + @X + @ForeignColumn + @Y
        ...
        SET @GeneralID = @VirtualID

        SET @SQLServerName = @SQLServerMore
