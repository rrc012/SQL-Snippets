...
-- generate SQL
  ...
  DECLARE Objects CURSOR FAST_FORWARD FOR
   SELECT E.GeneralID
        ...
        , E.SQLServerName
        , E.I
        , (SELECT MAX(T.column_id) FROM #TKey AS T WHERE T.GeneralID = E.GeneralID AND T.is_computed = 0 AND T.is_identity = 0) AS Z
     FROM #Base AS E
...
WHILE @@FETCH_STATUS = 0
    ...
    PRINT '   INSERT ' + @GeneralSchema + '.' + @X + @GeneralObject + @Y
    ...
    WHILE @@FETCH_STATUS = 0
        ...
        PRINT @SQLServerCode + @X + @GeneralColumn + @Y + CASE WHEN @T = @Z THEN ' )' ELSE SPACE(0) END
