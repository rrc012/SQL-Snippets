/*
 ===============================================================================
 Author:	     Phil Factor
 Source:       https://www.simple-talk.com/sql/t-sql-programming/visual-checks-data-distributed-sql-server/
 Article Name: Visual Checks on How Data is Distributed in SQL Server
 Create Date:  05-MAY-2017
 Description:  This script provides a visual representation of the way the data
               is distributed in tables.
			             
 Revision History:
 06-JUL-2017 - RAGHUNANDAN CUMBAKONAM
			Formatted the code.
			Added the history.
...
CREATE FUNCTION ColumnHistogram (@InVar IndependentVariable READONLY)  
...
DECLARE @variable TABLE(value INT),
	   ...
	   @MaxIndividualCount INT;

SELECT @Minimum = MIN(number),
       @Maximum = MAX(number)
  FROM @InVar;
...
CROSS JOIN
(
SELECT OneToForty.value AS x, SUM(IIF(g.value IS NOT NULL, 1, 0) AS frequency	
  FROM 
...
DECLARE @Variable IndependentVariable, --a simple table of numbers
        @Mean FLOAT, --the mean (and median if normally distributed)
        @StandardDeviation FLOAT, --(the Standard deviation you want)
        @ii INT, --counter
        @TotalWanted INT; --counter
